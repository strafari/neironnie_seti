...
class NumberProcessor():  # Объявляем класс NumberProcessor для обработки цифр
    def __init__(self, folder):  # Определяем метод инициализации класса
        ...
        self.learning_rate = 0.9  # Скорость обучения
        self.input_size = 10000  # Размер входного слоя (100x100 изображение -> 10000 пикселей)
        self.hidden_size = 16  # Размер скрытого слоя
    ...
    def show_weights(self):  # Метод для отображения весов
        fig, axes = plt.subplots(1, self.num_classes, figsize=(15, 5))
        for i in range(self.num_classes):
            weight_matrix = self.W2[:, i].reshape(int(np.sqrt(self.hidden_size)), -1)  # Преобразуем веса
            ax = axes[i]
...
class TestCanvas(tk.Tk):  # Объявляем класс TestCanvas для создания графического интерфейса
    def __init__(self, processor):  # Определяем метод инициализации класса
        ...
        show_button = tk.Button(self, text="Показать веса", command=processor.show_weights)  # Передаем метод объекта processor
        show_button.pack(side=tk.BOTTOM)  # Размещаем кнопку внизу окна
